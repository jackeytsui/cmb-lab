...
export async function buildVocalHackPlacementPreview() {
  ...
  const moduleIds = moduleRows.map((targetModule) => targetModule.id);
  const lessonRows =
    moduleIds.length > 0
      ? await db
          .select({
  ...
  const catalog: PlacementCatalog = {
    courses: courseRows,
    modules: moduleRows.map((targetModule) => ({
      ...targetModule,
      lessons: lessonRows
        .filter((lesson) => lesson.moduleId === targetModule.id)
        .map((lesson) => ({
          id: lesson.id,
          title: lesson.title,
...
export type VocalHackPlacementPreview = Awaited<
