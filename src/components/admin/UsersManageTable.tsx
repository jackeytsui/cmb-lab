...
export function UsersManageTable({
  ...
  const submitPortalAccess = async () => {
    ...
    setResultMessage(null);
    try {
      const res = await fetch("/api/admin/students/bulk-portal-access", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
        ...
        <div className="rounded-lg border border-primary/30 bg-primary/5 p-3 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex flex-wrap items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => openAction("portal_access")}
                className="inline-flex items-center gap-1.5 rounded-md border border-sky-500/30 bg-sky-500/10 px-3 py-1.5 text-xs font-medium text-sky-600 dark:text-sky-400 hover:bg-sky-500/20 transition-colors"
              >
          ...
          {activeAction === "portal_access" && (
            <div className="rounded border border-border bg-background p-3 space-y-2">
              <label className="text-xs font-medium text-muted-foreground">
                Set portal access for {selectedIds.size} user(s)
              </label>
              <select
              ...
              >
                ...
                <option value="paused">
                  Paused — cannot log in, data preserved
                </option>
                <option value="expired">
                  Expired — cannot log in, data preserved
                </option>
              </select>
