...
export function CoachingScheduleAdminClient() {
  ...
  const updateOccurrence = async (
    ...
    if (
      ...
      setError("Choose a date that belongs to this weekly series.");
      return;
    }
    if (
      cancelled &&
      !window.confirm(
        `Cancel “${event.title}” on ${dateKey}? Later dates will stay scheduled.`,
      )
    ) {
      return;
    }

    setOccurrenceSavingId(`${event.id}:${dateKey}`);
    setError(null);
    try {
      const response = await fetch(`/api/admin/coaching-events/${event.id}`, {
        method: "PUT",
        ...
        body: JSON.stringify({
          ...event,
          description: setCoachingOccurrenceCancelled(
            event.description,
            dateKey,
            cancelled,
          ),
      ...
      setOccurrenceDates((previous) => ({ ...previous, [event.id]: "" }));
      await load();
    } catch (reason) {
      setError(
    ...
    <div className="grid gap-6 lg:grid-cols-[24rem_minmax(0,1fr)]">
      ...
      <section className="space-y-3">
        ...
        {loading ? (
          ...
          events.map((event) => {
            const cancelledDates = cancelledCoachingDateKeys(event.description);
            const selectedDate = occurrenceDates[event.id] ?? "";
            return (
              <article
                key={event.id}
                ...
                {isWeeklyCoachingEvent(event.description) && (
                  <div className="mt-4 rounded-lg border border-border bg-muted/30 p-3">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Choose its Toronto date. The rest of the weekly series stays
                      scheduled.
                    </p>
                    <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                      <input
                        type="date"
                        aria-label={`Cancellation date for ${event.title}`}
                        min={localInputValue(event.startsAt).slice(0, 10)}
                        value={selectedDate}
                        onChange={(inputEvent) =>
                          setOccurrenceDates((previous) => ({
                            ...previous,
                      ...
                      <button
                        ...
                        disabled={
                          !selectedDate ||
                          occurrenceSavingId === `${event.id}:${selectedDate}`
                        }
                        onClick={() =>
                          void updateOccurrence(event, selectedDate, true)
                        }
                        className="inline-flex items-center justify-center gap-2 rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-700 disabled:opacity-50"
                      >
                        {occurrenceSavingId === `${event.id}:${selectedDate}` ? (
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {cancelledDates.map((dateKey) => (
                          <button
                            ...
                            type="button"
                            disabled={
                              occurrenceSavingId === `${event.id}:${dateKey}`
                            }
                            onClick={() =>
                              void updateOccurrence(event, dateKey, false)
                            }
                            className="inline-flex items-center gap-1.5 rounded-full border border-red-300 bg-red-50 px-2.5 py-1 text-xs font-medium text-red-700 hover:bg-red-100 disabled:opacity-50 dark:border-red-800 dark:bg-red-950/30 dark:text-red-300"
                            aria-label={`Restore ${event.title} on ${dateKey}`}
                          >
