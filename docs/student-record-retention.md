...
- Expired/paused access uses a durable Clerk ban, which revokes sessions. The portal labels remain `expired` / `paused`.
...
The job reads non-deleted student accounts only. It never restores records, reactivates access, changes GHL, or sends email. `?dryRun=true` reports proposed restrictions without mutations. Missing Clerk users and provider failures produce an error response rather than a false success; subsequent scheduled runs retry unresolved restrictions.
